...
workflow RUN {
    ...
    def custom_config = params.custom_config ? Channel.fromPath(params.custom_config).first() : Channel.fromPath("$projectDir/assets/empty.txt").first()).first() // need to pass a dummy file in the projectDir or similar
